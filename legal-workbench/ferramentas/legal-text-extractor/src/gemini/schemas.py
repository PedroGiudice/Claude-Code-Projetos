...
class SectionClassification(BaseModel):
    ...
    section_id: int = Field(
        ge=1,
        description="ID sequencial da seção (começando em 1)"
    )
    type: PecaType = Field(
        description="Tipo da peça processual (uma das 12 categorias)"
    )
    title: str = Field(
        min_length=1,
        max_length=200,
        description="Título ou descrição curta da seção"
    )
...
class ClassificationResult(BaseModel):
    ...
    total_pages: int = Field(
        ge=1,
        description="Total de páginas no documento"
    )
    total_sections: int = Field(
        ge=0,
        description="Total de seções identificadas"
    )
    sections: list[SectionClassification] = Field(
        description="Lista de seções classificadas"
    )
    summary: str = Field(
        min_length=1,
        max_length=500,
        description="Resumo do documento em 1-2 frases"
    )
    ...
    def validate_sections(self) -> 'ClassificationResult':
        ...
        for i, section in enumerate(self.sections):
            for j, other in enumerate(self.sections[i+1:], start=i+1):
                if not (section.end_page < other.start_page or
                        other.end_page < section.start_page):
                    # Verificar se é sobreposição real ou adjacência permitida
                    if section.end_page >= other.start_page and section.start_page <= other.end_page:
...
class CleanedSection(BaseModel):
    ...
    section_id: int = Field(
        ge=1,
        description="ID da seção (corresponde a SectionClassification.section_id)"
    )
    type: PecaType = Field(
        description="Tipo da peça (deve corresponder à classificação)"
    )
    content: str = Field(
        description="Texto limpo da seção (INTEGRAL, não resumido)"
    )
    noise_removed: list[str] = Field(
        default_factory=list,
        description="Exemplos de ruído removido (máximo 5)"
    )
...
class CleaningResult(BaseModel):
    ...
    sections: list[CleanedSection] = Field(
        description="Lista de seções limpas"
    )
    total_chars_original: int = Field(
        ge=0,
        description="Total de caracteres antes da limpeza"
    )
    total_chars_cleaned: int = Field(
        ge=0,
        description="Total de caracteres após limpeza"
    )
