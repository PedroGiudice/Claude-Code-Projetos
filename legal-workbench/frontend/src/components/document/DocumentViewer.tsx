...
export function DocumentViewer() {
    ...
    for (const event of allEvents) {
      ...
      if (event.type === 'annotationStart') {
        ...
        activeAnnotations = activeAnnotations.filter(
          (a) => a.fieldName !== (event.data as FieldAnnotation).fieldName
        );
        ...
        activePatterns = activePatterns.filter(
          (p) =>
            !(
  ...
  return (
    <div className="relative h-full">
      <div ref={containerRef} className="h-full overflow-y-auto px-8 py-6 bg-gh-bg-primary">
