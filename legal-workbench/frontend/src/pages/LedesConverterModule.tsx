...
const Icons = {
  File: ({ className, size = 24 }: { className?: string; size?: number }) => (
    <svg viewBox="0 0 24 24" fill="none" width={size} height={size} className={className}>
      <path d="M14 2H6C5.44772 2 5 2.44772 5 3V21C5 21.5523 5.44772 22 6 22H18C18.5523 22 19 21.5523 19 21V8L14 2Z" stroke="currentColor" strokeWidth="2.5" strokeLinecap="square" />
  ...
  Upload: ({ className, size = 24 }: { className?: string; size?: number }) => (
    <svg viewBox="0 0 24 24" fill="none" width={size} height={size} className={className}>
      <path d="M12 3L12 15M12 3L8 7M12 3L16 7" stroke="currentColor" strokeWidth="2.5" strokeLinecap="square" />
  ...
  Download: ({ className, size = 24 }: { className?: string; size?: number }) => (
    <svg viewBox="0 0 24 24" fill="none" width={size} height={size} className={className}>
      <path d="M12 3V15M12 15L8 11M12 15L16 11" stroke="currentColor" strokeWidth="2.5" strokeLinecap="square" />
  ...
  Settings: ({ className, size = 24 }: { className?: string; size?: number }) => (
    <svg viewBox="0 0 24 24" fill="none" width={size} height={size} className={className}>
      <rect x="2" y="6" width="20" height="2" stroke="currentColor" strokeWidth="2" strokeLinecap="square" />
      <rect x="2" y="16" width="20" height="2" stroke="currentColor" strokeWidth="2" strokeLinecap="square" />
  ...
  Check: ({ className, size = 24 }: { className?: string; size?: number }) => (
    <svg viewBox="0 0 24 24" fill="none" width={size} height={size} className={className}>
      <path d="M4 12L9 17L20 6" stroke="currentColor" strokeWidth="3" strokeLinecap="square" strokeLinejoin="miter" />
    </svg>
  ),
  ...
  Trash: ({ className, size = 24 }: { className?: string; size?: number }) => (
    <svg viewBox="0 0 24 24" fill="none" width={size} height={size} className={className}>
      ...
      <rect x="9" y="3" width="6" height="3" stroke="currentColor" strokeWidth="2.5" strokeLinecap="square" />
  ...
  Briefcase: ({ className, size = 24 }: { className?: string; size?: number }) => (
    <svg viewBox="0 0 24 24" fill="none" width={size} height={size} className={className}>
      <rect x="2" y="7" width="20" height="14" stroke="currentColor" strokeWidth="2.5" strokeLinecap="square" />
  ...
  Database: ({ className, size = 24 }: { className?: string; size?: number }) => (
    <svg viewBox="0 0 24 24" fill="none" width={size} height={size} className={className}>
      <path d="M21 5C21 6.65685 16.9706 8 12 8C7.02944 8 3 6.65685 3 5C3 3.34315 7.02944 2 12 2C16.9706 2 21 3.34315 21 5Z" stroke="currentColor" strokeWidth="2" />
  ...
  Calendar: ({ className, size = 24 }: { className?: string; size?: number }) => (
    <svg viewBox="0 0 24 24" fill="none" width={size} height={size} className={className}>
      <rect x="3" y="4" width="18" height="18" stroke="currentColor" strokeWidth="2.5" strokeLinecap="square" />
  ...
  Save: ({ className, size = 24 }: { className?: string; size?: number }) => (
    <svg viewBox="0 0 24 24" fill="none" width={size} height={size} className={className}>
      <path d="M19 21H5C4.44772 21 4 20.5523 4 20V4C4 3.44772 4.44772 3 5 3H19C19.5523 3 20 3.44772 20 4V20C20 20.5523 19.5523 21 19 21Z" stroke="currentColor" strokeWidth="2" strokeLinecap="square" />
  ...
  Logo: ({ className, size = 24 }: { className?: string; size?: number }) => (
      <svg viewBox="0 0 24 24" fill="none" width={size} height={size} className={className}>
        <rect x="2" y="2" width="20" height="20" stroke="currentColor" strokeWidth="2.5" strokeLinecap="square" />
...
const Button = ({
  ...
  const baseClasses = "flex items-center justify-center gap-2 px-5 py-3 rounded font-bold transition-all duration-150 active:translate-y-0.5 disabled:opacity-50 disabled:pointer-events-none text-xs tracking-wider uppercase font-['Manrope'] border";

  const variants = {
    primary: "bg-teal-500 hover:bg-teal-400 text-black border-teal-600 shadow-[2px_2px_0px_0px_rgba(20,184,166,0.3)]",
  ...
  return (
    <div className="flex flex-col gap-2 group">
      <div className="flex items-center justify-between">
          <label className={`text-[10px] font-bold flex items-center gap-2 transition-colors font-['Manrope'] uppercase tracking-widest ${required ? 'text-zinc-300' : 'text-zinc-500'} group-focus-within:text-violet-400`}>
            {label}
          </label>
          {required && (
              <div className="flex items-center gap-1.5 opacity-80">
                  <span className={`w-1 h-1 rounded-full ${isFilled ? 'bg-violet-500' : 'bg-amber-600 animate-pulse'}`}></span>
                  <span className={`text-[9px] font-mono tracking-tight uppercase ${isFilled ? 'text-violet-500' : 'text-amber-600/80'}`}>REQ</span>
              </div>
          )}
      </div>
...
const DataCard = ({ label, value, icon: Icon, highlight = false }: { label: string, value: string, icon: typeof Icons.File, highlight?: boolean }) => (
  <div className={`flex flex-col p-6 rounded-sm border transition-all duration-300 h-full justify-between ${highlight ? 'bg-teal-950/20 border-teal-500/40' : 'bg-[#121214] border-zinc-800 hover:border-zinc-700'}`}>
    <div className="flex items-center gap-2 mb-4 text-zinc-500">
      <Icon size={14} className={highlight ? "text-teal-400" : "text-zinc-600"} />
      <span className="text-[10px] font-bold uppercase tracking-widest font-['Manrope']">{label}</span>
    </div>
    <div className={`text-xl font-bold font-['JetBrains_Mono'] tracking-tight truncate ${value ? (highlight ? 'text-teal-300' : 'text-zinc-100') : 'text-zinc-600'}`}>
      {value || '---'}
    </div>
  </div>
);
...
export default function LedesConverterModule() {
  ...
  const handleFileUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected) {
      if (selected.name.endsWith('.docx') || selected.type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
        setFile(selected);
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-300 font-['Manrope'] selection:bg-teal-500/30 selection:text-teal-100 overflow-hidden flex flex-col relative">
      ...
      <header className="h-14 border-b border-zinc-800 bg-zinc-950/95 backdrop-blur-sm flex items-center justify-between px-6 shrink-0 z-20">
        ...
        <div className="flex items-center gap-4 text-xs">
          <div className={`flex items-center gap-2 px-3 py-1 border rounded-sm transition-colors ${isProcessing ? 'bg-lime-900/20 border-lime-800' : 'bg-zinc-900 border-zinc-800'}`}>
            <div className={`w-2 h-2 ${isProcessing ? 'bg-lime-400 animate-pulse' : 'bg-zinc-600'}`}></div>
            <span className={`font-bold uppercase tracking-wider ${isProcessing ? 'text-lime-400' : 'text-zinc-500'}`}>
      ...
      <main className="flex-1 flex overflow-hidden z-10">
        ...
        <div className="flex-1 flex flex-col h-full overflow-hidden relative">
          <div className="flex-1 overflow-y-auto p-8 lg:p-12 space-y-12 scroll-smooth">
            ...
            <section className="max-w-6xl mx-auto w-full">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className={`text-xl font-bold uppercase tracking-tight ${file ? 'text-teal-400' : 'text-zinc-200'}`}>
                      {file ? 'Source Acquired' : 'Upload Invoice'}
                    </h3>
                    {!file && <span className="text-[10px] text-zinc-500 font-bold bg-zinc-900 px-3 py-1.5 rounded-sm border border-zinc-800">.DOCX</span>}
                  </div>
              ...
              <div className="flex items-center gap-6 mt-8">
                ...
                {file && (
                  <Button
                    variant="secondary"
                    onClick={() => {reset(); setLogs(["System ready."]); addLog("Reset complete.");}}
                    icon={Icons.Trash}
              ...
              {!isConfigValid() && file && (
                <div className="mt-6 p-4 bg-amber-950/20 border-l-4 border-amber-500 text-amber-400 text-sm font-bold flex items-center gap-4">
                  <Icons.Alert size={20} />
                  <span>Warning: Missing required configuration fields. Fill out the config panel on the right.</span>
                </div>
              )}
            </section>
            ...
            <section className="max-w-6xl mx-auto w-full pt-10 border-t border-dashed border-zinc-800">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <DataCard
                  label="Total"
                  value={extractedData?.invoice_total ? `$${extractedData.invoice_total.toFixed(2)}` : ''}
                  icon={Icons.Money}
              ...
              <div className="mt-10 bg-[#0c0c0e] border border-zinc-800 rounded-sm overflow-hidden">
                  <div className="bg-[#18181b] px-5 py-4 border-b border-zinc-800 flex items-center justify-between">
                     <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">OUTPUT STRING</span>
                  ...
                  <div className="p-6 font-['JetBrains_Mono'] text-xs text-zinc-500 break-all leading-relaxed h-40 overflow-y-auto">
                    {extractedData ? (
                        <>
                            <span className="text-zinc-700 select-none mr-4 border-r border-zinc-800 pr-3">01</span>INVOICE_DATE|INVOICE_NUMBER|...<br/>
                            <span className="text-zinc-700 select-none mr-4 border-r border-zinc-800 pr-3">02</span>
          ...
          <div className={`shrink-0 bg-zinc-950 border-t border-zinc-800 transition-all duration-300 ease-in-out flex flex-col ${isLogExpanded ? 'h-64' : 'h-10'}`}>
             <button
             ...
             >
                <div className="flex items-center gap-3">
                    <Icons.Terminal size={14} className={isProcessing ? "text-lime-500" : "text-zinc-600"} />
                    <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest group-hover:text-zinc-400 transition-colors">System Log</span>
                ...
                {isLogExpanded ? <Icons.CaretDown size={14} className="text-zinc-600" /> : <Icons.CaretUp size={14} className="text-zinc-600" />}
             </button>

             <div className="flex-1 overflow-y-auto p-5 space-y-1 bg-[#050505]" style={{ scrollBehavior: 'smooth' }}>
                {logs.map((log, i) => (
        ...
        <aside className="w-96 bg-zinc-900 border-l border-zinc-800 flex flex-col h-full z-10 shadow-2xl shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-12">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-3 pb-3 border-b border-zinc-800/50">
                ...
                <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Firm Details</h3>
              </div>
              <InputField
                label="Firm ID"
                required
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-3 pb-3 border-b border-zinc-800/50">
                ...
                <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Client / Matter</h3>
              </div>
              <InputField
                label="Client ID"
                required
          ...
          <div className="p-8 border-t border-zinc-800 bg-zinc-900">
            <Button
              variant="secondary"
              onClick={saveConfig}
              className="w-full text-xs h-14 border-zinc-700 hover:border-violet-500 hover:text-violet-400 group"
              icon={Icons.Save}
            >
              SAVE CONFIGURATION
            </Button>
