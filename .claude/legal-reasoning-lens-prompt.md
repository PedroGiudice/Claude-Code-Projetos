...
## Implementação: Legal Reasoning Lens Output Style

---

<context>
Estou criando um Output Style especializado para raciocínio jurídico complexo no contexto brasileiro. Este estilo não transforma o Claude Code em "jurista" — ele já possui conhecimento jurídico. O objetivo é **focalizar** essas capacidades existentes como uma lupa, aplicando metodologia formal de argumentação jurídica.

**Fundamentos teóricos incorporados**:
- Raciocínio defeasible (Prakken, 1997) — normas admitem exceções e podem ser derrotadas
- Argumentação formal (ASPIC+, Dung's frameworks) — estrutura rigorosa de argumentos
- Case-based reasoning (HYPO, Ashley) — raciocínio por precedentes e dimensões
- Ontologias jurídicas (FOLaw, CLO) — categorização formal do conhecimento legal

**Contexto prático**:
- Taxa de alucinação em ferramentas legais: 17-33% (Lexis+, Westlaw)
- Verificação humana obrigatória para qualquer output jurídico
- Sistema brasileiro: hierarquia normativa, controle de constitucionalidade, jurisprudência vinculante

**Ambiente**: WSL2 Ubuntu 24.04

Os arquivos anexados contêm pesquisa acadêmica sobre AI & Law e relatório técnico de implementação.
</context>

<task>
Implementar o Output Style Legal Reasoning Lens:

1. Criar `~/.claude/output-styles/Legal-Reasoning-Lens.md`
2. Criar hook `~/.claude/hooks/legal-context.py`
...
## 1. Output Style: Legal-Reasoning-Lens.md

**Localização**: `~/.claude/output-styles/Legal-Reasoning-Lens.md`
...
## 2. Hook: legal-context.py

**Localização**: `~/.claude/hooks/legal-context.py`
...
Se já existe o Technical Director, fazer **merge**. Caso contrário, criar:

```json
{
  "outputStyle": "Legal Reasoning Lens",
  "hooks": {
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "python3 ~/.claude/hooks/legal-context.py"
          }
        ]
      }
    ]
...
**Se Technical Director já está instalado**, apenas criar o arquivo do Output Style. O usuário pode alternar entre eles com `/output-style`.
...
# 2. Criar Output Style
# [Conteúdo do Legal-Reasoning-Lens.md acima]

# 3. Criar hook
# [Conteúdo do legal-context.py acima]
chmod +x ~/.claude/hooks/legal-context.py

# 4. Verificar hook
echo '{}' | python3 ~/.claude/hooks/legal-context.py

# 5. Verificar se settings.json precisa ser atualizado
# (Se Technical Director já instalado, apenas criar o arquivo .md)

# 6. Listar output styles disponíveis
...
1. [ ] Output Style existe em `~/.claude/output-styles/Legal-Reasoning-Lens.md`
2. [ ] Frontmatter YAML válido com `name: Legal Reasoning Lens`
3. [ ] Hook executável e retorna JSON válido
4. [ ] `/output-style` lista o novo estilo
...
```bash
#!/bin/bash
echo "=== Legal Reasoning Lens Installation ==="

[ -f ~/.claude/output-styles/Legal-Reasoning-Lens.md ] && echo "✓ Output Style" || echo "✗ Output Style"
grep -q "IRAC" ~/.claude/output-styles/Legal-Reasoning-Lens.md 2>/dev/null && echo "✓ IRAC methodology" || echo "✗ IRAC missing"
grep -q "defeasible" ~/.claude/output-styles/Legal-Reasoning-Lens.md 2>/dev/null && echo "✓ Defeasible reasoning" || echo "✗ Defeasible missing"
[ -x ~/.claude/hooks/legal-context.py ] && echo "✓ Hook executable" || echo "✗ Hook not executable"
echo '{}' | python3 ~/.claude/hooks/legal-context.py 2>/dev/null | python3 -m json.tool > /dev/null && echo "✓ Hook output" || echo "✗ Hook output"
...
**Teste 1 — Questão jurídica simples**:
```
O prazo para contestação no procedimento comum é de quantos dias?
```
Esperado: Resposta direta com citação precisa (CPC art. 335), sem análise elaborada desnecessária.

**Teste 2 — Questão com complexidade argumentativa**:
```
Um contrato de locação comercial pode prever multa de 20% por atraso no pagamento do aluguel?
```
Esperado: Análise estruturada considerando liberdade contratual (CC), possível abusividade (CDC se aplicável), jurisprudência sobre redução de cláusula penal, com ressalvas sobre verificação.

**Teste 3 — Questão com lacuna/incerteza**:
```
Como se aplica a LGPD a dados de pessoas falecidas?
```
Esperado: Reconhecimento da lacuna legislativa, análise por analogia e princípios, referência a posições doutrinárias, indicação clara de incerteza.

**Teste 4 — Solicitação de citação específica**:
```
Qual o número do leading case do STJ sobre dano moral por negativação indevida?
```
Esperado: Ou cita com confiança alta, ou admite que não tem certeza do número exato e indica a tese geral. NUNCA fabrica número.

</behavioral_tests>

<usage_notes>

**Para alternar entre estilos**:
```
/output-style Technical Director    # Para trabalho técnico/código
/output-style Legal Reasoning Lens  # Para análise jurídica
```

**Quando usar este estilo**:
- Análise de questões jurídicas complexas
- Elaboração de argumentação formal
- Pesquisa jurídica estruturada
- Revisão de raciocínio legal

**Quando NÃO usar**:
- Tarefas puramente técnicas/código
- Formatação de documentos sem análise substantiva
- Trabalho operacional sem componente jurídico
