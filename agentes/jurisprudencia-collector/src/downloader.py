...
class DJENDownloader:
    ...
    def __init__(
        ...
        # Otimizado: 15 req/5s (180 req/min, 71% do limite da API de 21 req/5s)
        # Buffer de segurança 29% para evitar HTTP 429
        self.request_window_size = 15
        ...
        # HTTP session
        # OTIMIZAÇÃO P2: Headers mínimos (7% speedup - 288ms → 269ms)
        # Apenas Accept: application/json (User-Agent usa padrão do requests)
        ...
        if adaptive_rate_limit:
            req_per_min = int((self.request_window_size / self.request_window_duration) * 60)
            logger.info(
                f"Rate limiting adaptativo: {self.request_window_size} req/"
                f"{self.request_window_duration}s ({req_per_min} req/min, janela deslizante)"
            )
        else:
    ...
    def _check_rate_limit(self):
        ...
        - Limite: 15 requisições por janela de 5 segundos (180 req/min)
        - Buffer de segurança 29%: API permite ~21, usamos 15 (71% do limite)
        ...
        # BUGFIX: Incrementar DEPOIS de verificar janela (não antes)
        ...
        # Incrementar contador após verificação
        ...
        for tentativa in range(1, self.max_retries + 1):
            try:
                ...
                if response.status_code == 429:
                    # BUGFIX: Retry-After pode ser float (ex: 0.1)
                    retry_after_str = response.headers.get('Retry-After', '2')
                    try:
                        retry_after = float(retry_after_str)
                    except ValueError:
                        retry_after = 2.0  # Fallback se header for inválido

                    logger.warning(
                        f"HTTP 429 (Rate Limit) - "
                        f"Aguardando {retry_after:.1f}s antes de retry {tentativa}/{self.max_retries}"
                    )
                    time.sleep(retry_after)

                    # Resetar janela de rate limit após 429
                    # BUGFIX: Resetar completamente janela (HTTP 429 = limite excedido)
