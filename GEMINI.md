...
The main folder/project of the repo is: LEgal-Workbench (AKA "LW")

The project is primarily built with Python 3.11 and utilizes a set of libraries for data handling, PDF processing, and web scraping. All UIs for LW are React + Vite . It runs on a Linux environment, specifically Ubuntu 24.04 within WSL2.
...
The **Legal Workbench** is the primary, feature-rich web interface for the project. It is architected as an All-React Single-Page Application (SPA) frontend that communicates with a suite of FastAPI backend services. The entire environment is orchestrated with Docker.

**Services:**
*   **frontend-react:** The main React user interface.
*   **api-stj:** Service for STJ (Superior Tribunal de Justiça) integration.
*   **api-text-extractor:** Service for document text extraction.
*   **api-doc-assembler:** Service for assembling legal documents.
*   **api-trello:** Service for Trello integration.
*   **redis:** In-memory data store.

**Running Legal Workbench:**
The application is designed to be run via Docker Compose.
```bash
...
docker compose up -d
```
The main application is then accessible at `http://localhost/`.

### CCui (Claude Code UI)

The **CCui** is a more lightweight, specialized user interface designed for direct interaction with the Gemini AI Studio.
